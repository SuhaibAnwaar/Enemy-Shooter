...
AIceAmmo::AIceAmmo()
{
	...
	//Create Static Mesh
	IceComp = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("IceComp"));
	IceComp->SetupAttachment(RootComponent);
	IceComp->SetSimulatePhysics(false);
	IceComp->SetCollisionProfileName(TEXT("OverlapAll"));
	IceComp->SetWorldScale3D(FVector(.5f, 0.5f, 0.5f));

	//Create Ammo Text
	AmmoLabel = CreateDefaultSubobject<UTextRenderComponent>(TEXT("IceLabel"));
	AmmoLabel->SetupAttachment(IceComp);
	AmmoLabel->SetText(TEXT("Ice"));
...
void AIceAmmo::BeginPlay()
{
	Super::BeginPlay();
	
	//Call overlap function when the player overlaps the static mesh
	IceComp->OnComponentBeginOverlap.AddDynamic(this, &AIceAmmo::OnOverlapBegin);
...
// Called every frame
void AIceAmmo::Tick(float DeltaTime)
{
...
void AIceAmmo::OnOverlapBegin(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	if (PlayerRef == OtherActor)
	{
		...
		if (PlayerRef->ammotype == AmmoType::Ice)
		{
			...
			//If Smoke Ammo is below DefaultSmokeAmmo add  IceAmmo but clamp the IceAmmo
			//so the bullets added does not go over the max IceAmmo limit
			//then destroy itself
